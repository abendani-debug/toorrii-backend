...
class CategorieNiveau3Serializer(serializers.ModelSerializer):
    """Sous-catégories (niveau 3) — destinées à la sélection dans ServiceForm."""
    class Meta:
        model = Categorie
        fields = [
            "categorie_id",
...
class CategorieAfficherSerializer(serializers.ModelSerializer):
    """Secteurs (niveau 1) avec catégories et sous-catégories imbriquées — pour le select en cascade."""
    enfants = serializers.SerializerMethodField()
