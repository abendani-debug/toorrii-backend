...
class AfficherAboutNousView(APIView):
    """
    Endpoint pour afficher toutes les caractéristiques de l'entreprise (À propos de nous).
    Accessible par tous les utilisateurs.
    """
    permission_classes = [AllowAny]

    @extend_schema(
        summary="Afficher toutes les caractéristiques de l'entreprise",
        description="Retourne toutes les informations 'À propos de nous'. Les utilisateurs authentifiés "
                    "reçoivent toutes les données, le public reçoit une version limitée.",
        responses={
            200: OpenApiResponse(
                description="Liste des informations récupérée avec succès",
    ...
    def get(self, request):
        try:
            if request.user.is_authenticated:
                ...
                about_nous = AboutNous.objects.all().order_by('-date_creation')
                serializer = AboutNousAdminSerializer(about_nous, many=True)
            else:
                # Public → seulement les conditions actives
                about_nous = AboutNous.objects.filter(active=True)
                serializer = AboutNousPublicSerializer(about_nous, many=True)
